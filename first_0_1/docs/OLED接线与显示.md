...
OLED 与 MPU6050 共用重映射后的硬件 I2C1（PB8/PB9），软件自动尝试 OLED 的 7 位地址 0x3C 和 0x3D。总线为开漏，只保留一组适当的上拉电阻；若两个模块都自带强上拉，应核算并联阻值。PB10/PB11 已改为独立调试串口 USART3。

## 云台电机接口

第二块 TB6612 用于 K230 云台的两个直流电机：PA2/PA3 是 TIM2_CH3/CH4 PWM，PA11/PA12 是水平电机方向，PB0/PB1 是俯仰电机方向，PA5 是 STBY。底层驱动已提供 `MOTOR_GIMBAL_PAN` 和 `MOTOR_GIMBAL_TILT`，但尚未定义 K230 云台命令协议，因此 PA5 上电默认保持低电平。
...
每 250ms 尝试生成一帧，每次主循环最多传输 16 个显示字节，分块完成整屏。屏幕断开或总线异常时有界退出，每秒尝试重新连接一次。OLED 与 MPU6050 顺序访问同一条 PB8/PB9 硬件 I2C 总线。

若屏幕不亮，先检查实际驱动型号、四线接线及上拉，再看 Watch 的 `g_debugOledStatus`（1正常，3总线错误，4无应答）和 `g_debugOledAddress`。如果是 SH1106 或其他分辨率，需要按实际型号调整驱动，不能仅靠修改 I2C 地址替代。

## 验证

2026-09-15 重建 Keil Target 1：0 错误、0 警告，Flash 25000 字节、RAM 3992 字节。三种应用配置回归、MPU 启动测试、I2C 恢复测试、OLED 文本/状态测试和共享 I2C OLED 总线模型均通过。没有自动下载；实际 OLED 控制器兼容性、屏幕效果、共享总线时序以及电机转动仍待实机确认。
